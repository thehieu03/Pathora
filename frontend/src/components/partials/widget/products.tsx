  ...
  return (
    <div className="grid grid-cols-1 gap-5 md:grid-cols-3">
      {products.map((item, i) => (
        <div
          key={i}
          className="rounded-sm bg-slate-50 p-4 text-center dark:bg-slate-900"
        >
          ...
          <span className="mb-1 block text-sm font-normal text-slate-500 dark:text-slate-300">
            {item.price}
          </span>
          <span className="mb-4 block text-sm text-slate-600 dark:text-slate-300">
            {item.title}
          </span>
          <a
            href="#"
