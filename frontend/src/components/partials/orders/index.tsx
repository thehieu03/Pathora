...
const Orders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      setOrders(extractItems<any>(response.data));
    } catch (error) {
    ...
    const setupSignalR = async () => {
      try {
        connectionTimeout = setTimeout(() => {
          if (!signalRService.getConnected()) {
            console.warn("Orders: SignalR connection timeout, proceeding without real-time updates");
          }
        }, 10000);
        ...
        signalRService.onNotificationByKey("orders-page", (notification) => {
          ...
          if (
            ...
            orderService
              ...
              .catch((error) => {
                console.warn("Failed to fetch orders after notification:", error);
              });
          }
        });
      } catch (error) {
        console.warn("Failed to setup SignalR connection:", error);
      }
    };
        ...
        Cell: (row) => {
          ...
          return (
            <div className="flex items-center space-x-3">
              <div className="flex h-8 w-8 items-center justify-center rounded-full bg-slate-200 dark:bg-slate-600">
        ...
        Cell: (row) => {
          ...
          return (
            <div className="flex items-center justify-center gap-2">
              <span
                className={`inline-block min-w-[90px] rounded-[999px] px-3 py-1 text-center ${config.class}`}
              >
                {config.label}
              </span>
              {showInfoIcon && (
