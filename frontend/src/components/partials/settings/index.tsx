...
const Setings = () => {
  ...
  return (
    <div>
      {!isOpen && (
        <span
          className="shadow-deep fixed top-1/2 z-888 flex translate-y-1/2 rotate-90 transform cursor-pointer items-center bg-slate-800 px-2 py-2 text-sm font-medium text-slate-50 ltr:right-0 ltr:rounded-b md:ltr:right-[-32px] rtl:left-0 rtl:rounded-t md:rtl:left-[-32px] dark:bg-slate-700 dark:text-slate-300"
          onClick={() => setCustomizer(true)}
        >
          <Icon
            icon="clarity:settings-line"
            className="animate-spin text-lg text-slate-50"
          />
      ...
      <Transition as={Fragment} show={isOpen}>
        <div
          className="overlay fixed inset-0 z-999 bg-white/50"
          onClick={() => setCustomizer(false)}
        ></div>
      </Transition>
    </div>
