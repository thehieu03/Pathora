...
import type { CouponListModel } from "@/types/frontendModels";
import {
  useTable,
  useRowSelect,
  useSortBy,
  ...
  const fetchCoupons = useCallback(async () => {
    try {
      ...
      const mappedCoupons = couponList.map((item: CouponApiItem) => ({
        ...
        discount: item.discountValue || item.value,
        type: item.discountType || (item.type === 1 ? "fixed" : "percent"),
        displayType: item.discountType || item.displayType,
        minOrder: item.minOrderAmount || item.minPurchaseAmount || 0,
        maxDiscount: item.maxDiscountAmount,
        usageLimit: item.usageLimit || item.maxUsage,
        usedCount: item.usageCount || 0,
        startDate: item.startDate || item.validFrom,
        expiryDate: item.endDate || item.validTo,
        status:
          item.status ||
          mapStatus(item.displayStatus, item.isExpired, item.isOutOfUses),
        displayStatus: item.status || item.displayStatus,
  ...
  const handleViewClick = async (coupon) => {
    try {
      ...
      const mappedCoupon: CouponListModel = {
        ...
        discount: couponData.discountValue || couponData.value,
        type:
          couponData.discountType ||
          (couponData.type === 1 ? "fixed" : "percent"),
        displayType: couponData.discountType || couponData.displayType,
        minOrder:
          couponData.minOrderAmount || couponData.minPurchaseAmount || 0,
        maxDiscount: couponData.maxDiscountAmount,
        usageLimit: couponData.usageLimit || couponData.maxUsage,
        usedCount: couponData.usageCount || 0,
        startDate: couponData.startDate || couponData.validFrom,
        expiryDate: couponData.endDate || couponData.validTo,
        status:
          couponData.status ||
          mapStatus(
        ...
        displayStatus: couponData.status || couponData.displayStatus,
      ...
      console.warn("Failed to fetch coupon details");
    }
  };
    ...
    () => [
      {
        ...
        accessor: "code",
        Cell: (row) => (
          <span className="text-primary-500 bg-primary-500/10 rounded px-2 py-1 font-mono font-bold">
            {row?.cell?.value}
          </span>
        ),
      },
    ...
    <>
          ...
          <div className="space-y-4">
            <div className="mb-4 flex items-center justify-between">
              <div className="flex-1 text-center">
                <span className="text-primary-500 bg-primary-500/10 rounded-lg px-4 py-2 font-mono text-2xl font-bold">
                  {viewingCoupon.code}
                </span>
              </div>
            ...
            <div className="border-t border-slate-200 pt-4 dark:border-slate-700">
              <h6 className="mb-3 font-semibold text-slate-800 dark:text-slate-200">
      ...
      >
        {couponForValidity && (
          <div className="space-y-4">
            <div className="mb-4">
              <p className="mb-2 text-sm text-slate-500">{t("coupon.code")}</p>
